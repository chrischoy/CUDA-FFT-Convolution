...
__global__ void sumAlongFeaturesReduction(
    float *convolutionResult,
    volatile float *convolutionPerFeature,
    int FFT_H,
    int FFT_W,
    int FEATURE_DIM,
    ...
    if(x < FFT_W && y < FFT_H && z < PREV_POW_2_FEATURE_DIM){
        const int result_i = IMUL(FFT_H, x) + y;
        const int N = IMUL(FFT_W, FFT_H);
        int idx = PREV_POW_2_FEATURE_DIM;
        while(idx >= 1){
            idx = idx >> 1;
...
__global__ void sumAlongFeaturesReductionUnroll32(
    ...
    const int SharedN = IMUL(blockDim.x, blockDim.y);
    const int xyTid   = IMUL(threadIdx.y, blockDim.x) + threadIdx.x;
    ...
    if(x < FFT_W && y < FFT_H && z < PREV_POW_2_FEATURE_DIM){
        ...
        if (threadIdx.z + 16 < FEATURE_DIM)
            sdata[tid] = convolutionPerFeature[result_i + (  threadIdx.z *  N )] 
                       + convolutionPerFeature[result_i + (( threadIdx.z + 16 ) * N)];
        else
            sdata[tid] = convolutionPerFeature[result_i + (  threadIdx.z *  N )] ;
        ...
        if (threadIdx.z + 1 < PREV_POW_2_FEATURE_DIM) sdata[tid] +=sdata[tid + SharedN];__syncthreads();
        if (threadIdx.z == 0){
